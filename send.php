...
// ##### НАСТРОЙКИ #####
$from_email		 = 'promarmatura.by@gmail.com'; // от кого будет письмо
$recipient_email = 'promarmatura.by@gmail.com, san@kompensator.by'; // кому отправить письмо (можно несколько через запятую)
$reply_to_email = 'promarmatura.by@gmail.com'; // спец настройка почтового сервера "кому отправлять ответ" (обычно = $from_email)
...
$dictionary = [
    ...
    'Под приварку'=>'Welded end',
    'Межфланцевое'=>'Wafer',
    'Фланцевое'=>'Flanged',
    'Межфланцевое с резьбовыми проушинами'=>'Lug',
    'Межфланцевое с центрирующими проушинами'=>'Wafer',
    'Фланцевое с укороченной строительной длиной корпуса'=>'Flanged with short face-to-face length',
    'else'=>'Other',
...
// заполняем РФ
$sheet = $workbook->getSheet(3); // RU
fillCells3($sheet, $data);

// заполняем РБ
$sheet = $workbook->getSheet(4); // RU
fillCells3($sheet, $data);
...
function fillCells($sheet, $data) {
    ...
    $sheet->setCellValue('A2', $data['dn']['value']."--".$data['dn']['else']);
    $sheet->setCellValue('B2', $data['pressure']['dn']['value']);
    $sheet->setCellValue('C2', $data['standard']['value']);
    $sheet->setCellValue('D2', $data['design']['value']);
    $sheet->setCellValue('E2', $data['connection']['value']);
    $sheet->setCellValue('F2', $data['connection_standart']['value']);

    $face = $data['face']['value'];
    switch ($face) {
        case 'ГОСТ': $sheet->setCellValue('G2', $face."--".$data['face']['ГОСТ']['type']['value']);
            break;
        case 'EN': $sheet->setCellValue('G2', $face."--".$data['face']['EN']['type']['value']);
            break;
        case 'ASME': $sheet->setCellValue('G2', $face."--".$data['face']['ASME']['type']['value']);
            break;
    }
    $sheet->setCellValue('H2', $data['ftf']['value']);
    $sheet->setCellValue('I2', $data['body']['value']);
    $sheet->setCellValue('J2', $data['disc']['value']);
    $sheet->setCellValue('K2', $data['seat']['value']);
    $sheet->setCellValue('L2', $data['stem']['value']);
    $sheet->setCellValue('M2', $data['tightness']['value']);
    $sheet->setCellValue('N2', $data['working_pressure']['value']);
    $sheet->setCellValue('O2', $data['working_temperature']['value']);
    $sheet->setCellValue('P2', $data['working_ambient']['value']);
    $sheet->setCellValue('Q2', $data['valve_purpose']['value']); 
    $sheet->setCellValue('R2', $data['media']['value']);
    $sheet->setCellValue('S2', $data['operation']['value']);
    $sheet->setCellValue('T2', $data['quantity']['value']);
    $sheet->setCellValue('U2', $data['notes']['value']);


}


function fillCells3($sheet, $data) {

    
    $sheet->setCellValue('J10', $data['name_org']['value']);
    $sheet->setCellValue('J11', $data['name_proekt']['value']);
    $sheet->setCellValue('J12', $data['fio']['value']);
    $sheet->setCellValue('J13', $data['tech_position']['value']);
    $sheet->setCellValue('J14', $data['opros_list']['value']); 
...
function sendMail($filename) {
	...
	$message	 = 'Текстопросного листа'; //body of the email
